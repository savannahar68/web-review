...
class ScreenshotsCategoryRenderer extends CategoryRenderer {
  /**
  ...
  render(category, groupDefinitions = {}) {
    ...
    /*
    const img = this.dom.createElement('img','img-class', {src:"../screenshots/large.png"});
    categoryElem.appendChild(img);
    */
    this.templateContext = this.dom.document();
    this.detailsRenderer.setTemplateContext(this.templateContext);
    const imgt = this.dom.cloneTemplate("#tmpl-lh-ss", this.templateContext);

    const titleEl = this.dom.find('#ss-container-title', imgt);
    titleEl.textContent = 'The screenshots have been resized to fit on the page. Click on the title to open image in original size.';

    const images = category.images;
    images.forEach(img => {
      const img_elem = this.dom.cloneTemplate(".ss-lg-indi", this.templateContext);

      const imgTitleEl = this.dom.find('.indi-ss-title-link', img_elem);
      imgTitleEl.textContent = img.title;
      imgTitleEl.href = img.src;

      const imgBodyEl = this.dom.find('.indi-img', img_elem);
      imgBodyEl.src = img.src;

      imgt.appendChild(img_elem);
    });

    categoryElem.appendChild(imgt);
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = ScreenshotsCategoryRenderer;
} else {
  self.ScreenshotsCategoryRenderer = ScreenshotsCategoryRenderer;
}
